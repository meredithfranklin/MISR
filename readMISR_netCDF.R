...
setwd("/Volumes/Projects-2/Satellite/MISR/")

#misr.files <- list.files("./V22b24-86/",pattern="*-86.b060",full.names=TRUE)
misr.files <- list.files("./LM_4km/",pattern="*.nc",full.names=TRUE)
...
for(i in 1:1) { #length(misr.files)
  dat<-open.nc(misr.files[i])
  AOD<-var.get.nc(dat, "Aerosol_Optical_Depth")
